...
function getProById($id){
		$sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName,p.cId from imooc_pro as p join MFY_cate c on p.cId=c.id where p.id={$id}";
		$row=fetchOne($sql);
		return $row;
}

function getProsByCid($cid){
		$sql="select p.id,p.pName,p.iPrice from MFY_pro as p join MFY_cate c on p.cId=c.id where p.cid={$cid} limit 4";
		$rows=fetchAll($sql);
		return $rows;
}
...
function getSmallProsByCid($cid){
		$sql="select p.id,p.pName,p.iPrice from MFY_pro as p join MFY_cate c on p.cId=c.id where p.cid={$cid} limit 4,4";
		$rows=fetchAll($sql);
		return $rows;
}
